...
def parse_args():
	...
	parser.add_argument("--warmup_steps", type=int, default=20, help="Number of warmup inference steps after loading new model.")
	parser.add_argument("--warmup_spp", type=int, default=4, help="Samples per pixel during warmup (lower for speed).")
...
def warmup_model(testbed, warmup_steps, warmup_spp, resolution):
	"""
	Perform warmup inference to stabilize the model after loading new snapshot.
	This helps with density grid updates and internal state synchronization.
	"""
	print(f"  Warming up model with {warmup_steps} inference steps...")
	
	for step in range(warmup_steps):
		# Perform inference at random camera positions to warm up the model
		# Use lower spp for speed during warmup
		random_t = np.random.random()
		
		try:
			# Render a warmup frame (don't save it)
			warmup_frame = testbed.render(
				resolution[0], resolution[1], 
				warmup_spp,  # Lower SPP for speed
				True,  # accumulate
				random_t, random_t,  # Random position along camera path
				30,  # fps (not important for warmup)
				shutter_fraction=0.5
			)
			
			# Optional: Force some internal updates by accessing frame
			_ = warmup_frame.mean()  # Simple operation to ensure frame is computed
			
		except Exception as e:
			print(f"    Warning: Warmup step {step} failed: {e}")
			continue
...
if __name__ == "__main__":
	...
	if args.video_camera_path:
		...
		# ===== MAIN CHANGE: Single testbed with adaptive warmup =====
		...
		print(f"Rendering {n_frames} frames with model switching and warmup...")
		print(f"Warmup settings: {args.warmup_steps} steps, {args.warmup_spp} SPP")
		...
		for i in tqdm(list(range(min(n_frames, n_frames+1))), unit="frames", desc=f"Rendering video"):
			...
			if new_model_id != current_model_id:
				...
				# NEW: Warmup the model after loading new snapshot
				if args.warmup_steps > 0:
					warmup_model(testbed, args.warmup_steps, args.warmup_spp, resolution)
				
				current_model_id = new_model_id
			
			# Render frame (testbed is now properly warmed up)
